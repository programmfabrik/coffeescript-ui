...
class CUI.Layer extends CUI.DOM
	...
	position: (ev) ->
		...
		set_css =
			...
			minWidth: minWidth
			maxWidth: Math.floor(vp.width + vp.overlap_width - (vp.layer_pos.left - vp.window_left))
			maxHeight: Math.floor(vp.height + vp.overlap_height - (vp.layer_pos.top - vp.window_top))
			margin: 0
